...
public class AdvancementWidgetMixin implements IAdvancementWidget {
    ...
    @Override
    public void moonstone1$draw(GuiGraphics guiGraphics, int x, int y) {
        ...
        for (AdvancementWidget advancementwidget : this.children) {
            if (advancementwidget instanceof IAdvancementWidget advancementWidget) {
                advancementWidget.moonstone1$draw(guiGraphics, x, y);
            }
        }
    ...
    @Override
    public void moonstone1$drawHover(GuiGraphics guiGraphics, int x, int y, float fade, int width, int height) {
        ...
        if (!this.description.isEmpty()) {
            if (flag1) {
                guiGraphics.blitSprite(ResourceLocation.fromNamespaceAndPath(MoonStoneMod.MODID,"advancements/box"), i1, l + 26 - j1, this.width, j1);
            } else {
                guiGraphics.blitSprite(ResourceLocation.fromNamespaceAndPath(MoonStoneMod.MODID,"advancements/box"), i1, l, this.width, j1);
            }
        }
    ...
    @Override
    public void moonstone1$drawNecora(GuiGraphics guiGraphics, int x, int y) {
        if (!this.display.isHidden() || this.progress != null && this.progress.isDone()) {
            float f = this.progress == null ? 0.0F : this.progress.getPercent();
            NecoraWidgetTypes advancementwidgettype;
            if (f >= 1.0F) {
                advancementwidgettype = NecoraWidgetTypes.OBTAINED;
            } else {
                advancementwidgettype = NecoraWidgetTypes.UNOBTAINED;
            }
        ...
        for (AdvancementWidget advancementwidget : this.children) {
            if (advancementwidget instanceof IAdvancementWidget advancementWidget) {
                advancementWidget.moonstone1$drawNecora(guiGraphics, x, y);
            }
        }
    ...
    @Override
    public void moonstone1$drawHoverNecora(GuiGraphics guiGraphics, int x, int y, float fade, int width, int height) {
        boolean flag = width + x + this.x + this.width + 26 >= this.tab.getScreen().width;
        Component component = this.progress == null ? null : this.progress.getProgressText();
        int i = component == null ? 0 : this.minecraft.font.width(component);
        boolean flag1 = 113 - y - this.y - 26 <= 6 + this.description.size() * 9;
        ...
        int j = Mth.floor(f * (float)this.width);
        NecoraWidgetTypes advancementwidgettype;
        NecoraWidgetTypes advancementwidgettype1;
        NecoraWidgetTypes advancementwidgettype2;
        if (f >= 1.0F) {
            j = this.width / 2;
            advancementwidgettype = NecoraWidgetTypes.OBTAINED;
            advancementwidgettype1 = NecoraWidgetTypes.OBTAINED;
            advancementwidgettype2 = NecoraWidgetTypes.OBTAINED;
        } else if (j < 2) {
            j = this.width / 2;
            advancementwidgettype = NecoraWidgetTypes.UNOBTAINED;
            advancementwidgettype1 = NecoraWidgetTypes.UNOBTAINED;
            advancementwidgettype2 = NecoraWidgetTypes.UNOBTAINED;
        } else if (j > this.width - 2) {
            j = this.width / 2;
            advancementwidgettype = NecoraWidgetTypes.OBTAINED;
            advancementwidgettype1 = NecoraWidgetTypes.OBTAINED;
