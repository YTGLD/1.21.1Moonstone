...
public class BundleContentsDNA {
    public static final BundleContentsDNA EMPTY = new BundleContentsDNA(List.of());
    public static final Codec<BundleContentsDNA> CODEC;
    public static final StreamCodec<RegistryFriendlyByteBuf, BundleContentsDNA> STREAM_CODEC;
    private static final Fraction BUNDLE_IN_BUNDLE_WEIGHT;
    private static final int NO_STACK_INDEX = -1;
    final List<ItemStack> items;
    final Fraction weight;

    BundleContentsDNA(List<ItemStack> items, Fraction weight) {
        this.items = items;
        this.weight = weight;
    }

    public BundleContentsDNA(List<ItemStack> items) {
    ...
    static Fraction getWeight(ItemStack stack) {
        ...
        if (bundlecontents != null) {
            ...
            return !list.isEmpty() ? Fraction.ONE : Fraction.getFraction(1,1024);
        }
    }
    ...
    public boolean equals(Object other) {
        if (this == other) {
            ...
            boolean var10000;
            if (other instanceof BundleContentsDNA) {
                BundleContentsDNA bundlecontents = (BundleContentsDNA)other;
    ...
    static {
        CODEC = ItemStack.CODEC.listOf().xmap(BundleContentsDNA::new, (p_331551_) -> {
            return p_331551_.items;
        });
        STREAM_CODEC = ItemStack.STREAM_CODEC.apply(ByteBufCodecs.list()).map(BundleContentsDNA::new, (p_331649_) -> {
    ...
    public static class Mutable {
        ...
        public Mutable(BundleContentsDNA contents) {
            this.items = new ArrayList(contents.items);
            this.weight = contents.weight;
        }

        public BundleContentsDNA.Mutable clearItems() {
            this.items.clear();
        ...
        private int getMaxAmountToAdd(ItemStack stack) {
            ...
            return Math.min(Math.max(fraction.divideBy(BundleContentsDNA.getWeight(stack)).intValue(), 0), 64);
        }
        ...
        public int tryInsert(ItemStack stack) {
            if (!stack.isEmpty() && stack.getItem().canFitInsideContainerItems()) {
                ...
                int availableSpace = 1024 - totalItems;
                ...
                if (i == 0) {
                    ...
                    this.weight = this.weight.add(BundleContentsDNA.getWeight(stack).multiplyBy(Fraction.getFraction(i, 1)));
        ...
        public ItemStack removeOne() {
            if (this.items.isEmpty()) {
                ...
                this.weight = this.weight.subtract(BundleContentsDNA.getWeight(itemstack).multiplyBy(Fraction.getFraction(itemstack.getCount(), 1)));
                return itemstack;
            }
        }
        ...
        public BundleContentsDNA toImmutable() {
            return new BundleContentsDNA(List.copyOf(this.items), this.weight);
        }
    }
}
