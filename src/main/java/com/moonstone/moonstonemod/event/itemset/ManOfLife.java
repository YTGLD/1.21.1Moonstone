...
public class ManOfLife {
    public static void healEvent(LivingHealEvent event){
        if (Handler.hasItemInList(event.getEntity(), Config.SERVER.listManOfLife.get())){
    ...
    public static void attackEvent(LivingIncomingDamageEvent event){
        if (event.getSource().getEntity()!=null
            ...
            if (Handler.hasItemInList(player, Config.SERVER.listManOfLife.get())) {
                event.setAmount(event.getAmount()*1.05f);
            }
        }
    }
    public static void criticalHitEvent(CriticalHitEvent event){
        ...
        if (Handler.hasItemInList(player, Config.SERVER.listManOfLife.get())) {
            event.setDamageMultiplier(event.getDamageMultiplier() * 1.05f);
        }
    }
    public static void health(Player player, CallbackInfoReturnable<Float> cir){
        if (Handler.hasItemInList(player, Config.SERVER.listManOfLife.get())){
            cir.setReturnValue(cir.getReturnValue()* 1.05f);
        }
    }
